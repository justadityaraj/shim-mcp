...
- **Credential-shaped options never cross the wire.** The option abilities refuse to read or write any option whose name looks like a stored credential (API keys, secrets, tokens, passwords, salts, `connectors_ai_*`), and the option search returns names only, never values.
