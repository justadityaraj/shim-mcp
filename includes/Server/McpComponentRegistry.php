...
use ShimMcp\Server\Domain\Prompts\Contracts\McpPromptBuilderInterface;
use ShimMcp\Server\Domain\Prompts\McpPrompt;
use ShimMcp\Server\Domain\Prompts\RegisterAbilityAsMcpPrompt;
use ShimMcp\Server\Domain\Resources\McpResource;
use ShimMcp\Server\Domain\Resources\RegisterAbilityAsMcpResource;
use ShimMcp\Server\Domain\Tools\McpTool;
use ShimMcp\Server\Domain\Tools\RegisterAbilityAsMcpTool;
...
class McpComponentRegistry {
	 ...
	 * Constructor.
	 *
	 * @param \ShimMcp\Server\McpServer $mcp_server MCP server instance.
	 * @param \ShimMcp\Server\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface $error_handler Error handler instance.
	 * @param \ShimMcp\Server\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface $observability_handler Observability handler instance.
	...
	public function register_tools( array $tools ): void {
		foreach ( $tools as $tool_item ) {
			...
			if ( $this->mcp_validation_enabled ) {
				...
				$validation_result = \ShimMcp\Server\Domain\Tools\McpToolValidator::validate_tool_instance( $tool, $context_to_use );
	 ...
	 *
	 * @param \ShimMcp\Server\Domain\Tools\McpTool $tool The tool instance to register.
	 *
	 * @return void
	 */
	...
	public function register_prompts( array $prompts ): void {
		foreach ( $prompts as $prompt_item ) {
			...
			if ( class_exists( $prompt_item ) && in_array( McpPromptBuilderInterface::class, class_implements( $prompt_item ) ?: array(), true ) ) {
				...
				try {
					/** @var \ShimMcp\Server\Domain\Prompts\Contracts\McpPromptBuilderInterface $builder */
				...
				if ( $this->mcp_validation_enabled ) {
					...
					$validation_result = \ShimMcp\Server\Domain\Prompts\McpPromptValidator::validate_prompt_instance( $prompt, $context_to_use );
	 ...
	 *
	 * @return \ShimMcp\Server\Domain\Tools\McpTool[]
	 */
	 ...
	 *
	 * @return \ShimMcp\Server\Domain\Prompts\McpPrompt[]
	 */
